...
public final class RemoteServiceParametersHelper {
    ...
    private final JSONArray buildEventsJson(List<AppEvent> appEvents, String applicationId) {
        ...
        try {
            ...
            List<AppEvent> listX = CollectionsKt___CollectionsKt.X(appEvents);
            EventDeactivationManager.processEvents(listX);
            boolean zIncludeImplicitEvents = includeImplicitEvents(applicationId);
            for (AppEvent appEvent : listX) {
                if (!appEvent.isChecksumValid()) {
                    Utility utility = Utility.INSTANCE;
                    Utility.logd(TAG, s.m("Event with invalid checksum: ", appEvent));
                } else if ((!appEvent.isImplicit()) || (appEvent.isImplicit() && zIncludeImplicitEvents)) {
